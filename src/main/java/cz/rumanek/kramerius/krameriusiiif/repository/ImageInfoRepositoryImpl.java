...
public class ImageInfoRepositoryImpl implements ImageInfoRepository {
    ...
    private ThreadLocal<URI> request = new ThreadLocal<>();
    ...
    public Info getInfo(String pid) {
        try {
            ...
            logger.trace("START:" + pid + ":");
            URI requestURI = new URI("/" + pid + "/" + INFO_JSON);
            request.set(requestURI);
            Info info = restTemplate.getForObject(requestURI.toString(), Info.class);
            logger.trace("FINISH:" + pid + ":" + info.toString() + ":" + (System.currentTimeMillis() - start));
            return info;
        }
        catch (HttpStatusCodeException e) {
            logger.error("Invalid request => " + rootUri + request.get() + " => " + e.getStatusText() + " => " + e.getStatusCode()
                    + System.lineSeparator() + e);
            throw e;
        }
        catch (RestClientException e) {
            logger.error("RestClientException => " + rootUri + request.get() + " => " + System.lineSeparator() + e);
            throw e;
        }
        catch (Exception e) {
            logger.error("Exception in request => " + request.get() + System.lineSeparator() + e);
            return null;
        }
    }
}
